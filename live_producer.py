...
def create_process_message(symbol):
    ...
    def process_message(msg):
        try:
            # Only process messages that have the 'sequence' attribute.
            if not hasattr(msg, 'sequence'):
                # Log or ignore control/metadata messages
                print(f"Ignoring non-MBP message for {symbol}: {msg}")
                return
...
def run_client(client, symbol):
    ...
    try:
        ...
        # client.block_for_close(timeout=900)  # Run for 5 minutes.
