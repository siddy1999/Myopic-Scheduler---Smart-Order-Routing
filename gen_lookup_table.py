...
def generate_xi(outflows, T, method="exp") -> np.ndarray:
    ...
    # Compute distribution parameters using the combined function
...
class StochasticOrderOptimizer:
    ...
    def calculate_gradient(self, X: np.ndarray, xi: np.ndarray) -> np.ndarray:
        ...
        for k in range(self.n_exchanges):
            indicator_isfilled = 1. if (xi[k] > self.params['Q'][k] + L[k]) else 0.
...
def execute_optimization(
    ...
    *,
    ...
    if method == "lookup":
        ...
        final_ratio = lookup_table[composite_key]["avg_ratio"]
        print(f"For minute {minute_before}, composite key '{composite_key}' yields ratio vector: {final_ratio}")
        return (final_ratio*S, None)
    else:
